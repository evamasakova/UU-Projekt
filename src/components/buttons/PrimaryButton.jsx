...
export default function PrimaryButton({ onClick, children, type = 'button', className = '', icon = null, variant = 'dark' }) {
	const variantClass = variant === 'light' ? 'primary-button primary-button--light' : 'primary-button'
	const hasText = children !== undefined && children !== null && children !== ''
	const hasIcon = !!icon

	const gapClass = hasIcon && hasText ? 'gap-2' : 'gap-0'
	const sizeClass = hasIcon && !hasText

...
export default function DetailPageHeader({ name, category }) {
  ...
  const handleRate = async (value) => {
    try {
      ...
      const newAvg = userRating ? (avg * count - userRating + value) / count : (avg * count + value) / newCount;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto py-5">
      <header className="flex items-start justify-between">
        <div>
          <h1 className="text-4xl font-extrabold text-gray-900 leading-tight">{name}</h1>
          <p className="mt-3">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-center gap-2">
            <div className="flex items-center">{displayStars(userRating ?? Math.round(avg))}</div>
            <div className="ml-3 text-sm text-gray-700">
              <div className="font-medium">{avg ?? 0} / 5</div>
              <div className="text-xs text-gray-500">{count} rating{count === 1 ? "" : "s"}</div>
            </div>
          </div>
          {userRating && <div className="text-xs text-gray-500">You rated: {userRating}★</div>}
        </div>
